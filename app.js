...
function buildAirlabsSchedulesUrl(icao) {
  ...
  const sonometersEBLG = [
    ...
    { id: "F015", address: "Rue du Brouck, Juprelle", latDMS: "50 41 19.82 N", lonDMS: "5 31 34.38 E" },
    { id: "F005", address: "Rue Caquin, Haneffe", latDMS: "50 38 21.59 N", lonDMS: "5 19 24.67 E" },
...
function filterFlightsAirlabsForNoise(airportKey, flights) {
  ...
  function startTimeline() {
    ...
    timelineTimer = setInterval(() => {
      ...
      document.getElementById("timeline-range").value = timelineCurrent;
  ...
  function onFlightSelected(airportKey, key, kind) {
    ...
    if (!fm) {
      infoEl.textContent = "Vol introuvable dans l’index.";
      sonosEl.textContent = "";
      return;
    }
    ...
    if (!proc) {
      sonosEl.textContent = "Aucune procédure SID/STAR associée à ce vol.";
      return;
    }

    const impacted = computeImpactedForFlight(airportKey, proc, f);
    if (!impacted.length) {
      sonosEl.textContent = "Aucun sonomètre dans le couloir de ce vol.";
      return;
    }
